...
const SheetData = (() => {
  ...
  function parseTeamSheet(rows) {
    ...
    let mode = null;
    let remainingCapFromSheet = null;
    let deadCapFromSheet = null;

    for (const r of rows) {
      const c1 = (r[1] || "").trim();
      const c8 = (r[8] || "").trim();

      // The team's remaining cap space is printed near the top of its tab
      // (column 8, before "Active Roster" begins) and its dead cap total
      // sits next to the "Dead Cap" label in the Active Roster header row.
      // Reading these directly from the team's own tab avoids having to
      // name-match this team against a separate "Snapshot" tab at all.
      if (mode === null && remainingCapFromSheet === null && /^\$?-?[\d,.]+$/.test(c8)) {
        remainingCapFromSheet = money(c8);
      }
      if (c8 === "Dead Cap") {
        deadCapFromSheet = money(r[9]);
      }
    ...
    return { ...sections, totals, deadCapPlayers, remainingCapFromSheet, deadCapFromSheet };
  }
  ...
  async function loadAll(sheetId, sheetTabsByRosterId, snapshotTabName) {
    ...
    rosterIds.forEach((rid, idx) => {
      ...
      const parsed = result.status === "fulfilled"
        ...
        : { activeRoster: [], taxiSquad: [], ir: [], deadCapPlayers: [], totals: { activeRosterSalary: 0, taxiSalary: 0, irSalary: 0 }, remainingCapFromSheet: null, deadCapFromSheet: null, loadFailed: true };
      // Prefer figures read directly from the team's own tab; fall back to
      // the Snapshot tab (matched by name) only if that team's tab didn't
      // parse (e.g. it failed to load).
      const snap = snapshot[tabName];
      const remainingCap = parsed.remainingCapFromSheet !== null ? parsed.remainingCapFromSheet : (snap ? snap.remainingCap : 0);
      const deadCap = parsed.deadCapFromSheet !== null ? parsed.deadCapFromSheet : (snap ? snap.deadCap : 0);
      byRoster[rid] = {
        ...parsed,
        cap: {
